document.querySelectorAll(".secureLink").forEach(function (element) {
  element.addEventListener("click", function (event) {
    ...
    if (secureWindow) {
      // Load the content from the external HTML file using XMLHttpRequest
      let xhr = new XMLHttpRequest();
      xhr.open("GET", "secureWindowContent.html", true);
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
          // Write the content to the new window
          secureWindow.document.open();
          secureWindow.document.write(xhr.responseText.replace("{{URL}}", url));
